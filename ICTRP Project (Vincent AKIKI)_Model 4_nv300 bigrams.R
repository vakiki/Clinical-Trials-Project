...
train.tokens <- tokens_ngrams(train.tokens, n = 2)
...
# saveRDS(test.svd, "./M4_test.svd.rds")
...
M4_rf.cv.bigram.nv300.nolength <- train(Label ~ ., data = train.svd, method = "rf",
...
saveRDS(M4_rf.cv.bigram.nv300.nolength, "./M4_rf.cv.bigram.nv300.nolength.rds")

# You can load the model next time without running again the machine learning step.
# M4_rf.cv.bigram.nv300.nolength <- readRDS("./M4_rf.cv.bigram.nv300.nolength.rds")
# print(M4_rf.cv.bigram.nv300.nolength)

# Prediction result
M4_rf.cv.bigram.nv300.nolength

preds <- predict(M4_rf.cv.bigram.nv300.nolength, test.svd)
