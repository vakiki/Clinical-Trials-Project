...
#STEP 5 : PERFORM SINGULAR VALUE DECOMPOSITION (SVD) ON THE TF-IDF TO REDUCE FEATURES NUMBER TO 500 FOR OUR LATENT SEMANTIC ANALYSIS (LSA).

# Time the code execution
start.time <- Sys.time()

# Perform SVD to reduce dimensionality down to 500 columns
train.irlba <- irlba(t(train.tokens.DFM.TFIDF_matrix), nv = 500, maxit = 600)
...
# Create new feature data frame using our document semantic space of 500 features
...
# saveRDS(test.svd, "./M2_test.svd.rds")
...
rf.cv.unigram.nv500.nolength <- train(Label ~ ., data = train.svd, method = "rf",
...
saveRDS(rf.cv.unigram.nv500.nolength, "./rf.cv.unigram.nv500.nolength.rds")

# You can load the model next time without running again the machine learning step.
# rf.cv.unigram.nv500.nolength <- readRDS("./rf.cv.unigram.nv500.nolength.rds")
# print(rf.cv.unigram.nv500.nolength)

# Prediction result
rf.cv.unigram.nv500.nolength

preds <- predict(rf.cv.unigram.nv500.nolength, test.svd)
