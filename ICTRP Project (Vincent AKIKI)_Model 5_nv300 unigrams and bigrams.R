...
###############################################################
# WHO CLINICAL PORTAL PROJECT - Vincent AKIKI - January 2022  #
###############################################################
...
# Add bigrams features to the unigrams features
...
# The data set is too large and too sparse :
# reduce number of features by triming/ removing features occuring :
#       - less than 2 times overall (min_termfreq)
#       - in less than 2 documents (min_docfreq)

train.tokens.DFM <-  dfm_trim(train.tokens.DFM, min_termfreq = 2, min_docfreq = 2, verbose = TRUE)
...
# saveRDS(test.svd, "./M5_test.svd.rds")
...
rf.cv.unigram.bigram.nv300.nolength <- train(Label ~ ., data = train.svd, method = "rf",
...
saveRDS(rf.cv.unigram.bigram.nv300.nolength, "./rf.cv.unigram.bigram.nv300.nolength.rds")

# You can load the model next time without running again the machine learning step.
# rf.cv.unigram.bigram.nv300.nolength <- readRDS("./rf.cv.unigram.bigram.nv300.nolength.rds")
# print(rf.cv.bigram.nv300.nolength)

# Prediction result
rf.cv.unigram.bigram.nv300.nolength

preds <- predict(rf.cv.unigram.bigram.nv300.nolength, test.svd)
