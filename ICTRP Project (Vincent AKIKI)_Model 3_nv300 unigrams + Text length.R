...
# Adding the text length we calculate previously to our 300 SVM features
train.svd$Pub_TextLength <- train$Pub_TextLength
train.svd$Sci_TextLength <- train$Sci_TextLength
train.svd$Cond_TextLength <- train$Cond_TextLength
...
test.svd <- data.frame(Label = test$Label, test.svd.raw,
                       Pub_TextLength = test$Pub_TextLength,
                       Sci_TextLength = test$Sci_TextLength,
                       Cond_TextLength = test$Cond_TextLength)

# saveRDS(test.svd, "./M3_test.svd.rds")
...
rf.cv.unigram.nv300.withlength <- train(Label ~ ., data = train.svd, method = "rf",
...
saveRDS(rf.cv.unigram.nv300.withlength, "./rf.cv.unigram.nv300.withlength.rds")

# You can load the model next time without running again the machine learning step.
# rf.cv.unigram.nv300.withlength <- readRDS("./rf.cv.unigram.nv300.withlength.rds")
# print(rf.cv.unigram.nv300.withlength)

# Prediction result
rf.cv.unigram.nv300.withlength

preds <- predict(rf.cv.unigram.nv300.withlength, test.svd)
